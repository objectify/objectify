...
public class ClassSaver implements Saver
{
	 ...
	 */
	public ClassSaver(Conversions conv, String pathPrefix, Class<?> clazz, boolean ignoreClassIndexing, boolean collectionize, boolean embedding)
	{
		...
		for (FieldMetadata metadata: fields)
		{
			...
			if (TypeUtils.isEmbedded(field))
			{
				...
				{
					Saver saver = new EmbeddedArrayFieldSaver(conv, pathPrefix, clazz, field, ignoreClassIndexing, collectionize);
					this.fieldSavers.add(saver);
				}
				else if (Collection.class.isAssignableFrom(field.getType()))
				{
					Saver saver = new EmbeddedCollectionFieldSaver(conv, pathPrefix, clazz, field, ignoreClassIndexing, collectionize);
					this.fieldSavers.add(saver);
				}
				else	// basic class
				{
					Saver saver = new EmbeddedClassFieldSaver(conv, pathPrefix, clazz, field, ignoreClassIndexing, collectionize);
					this.fieldSavers.add(saver);
				}
			}
			...
			{
				// Add a leaf saver
				Saver saver = new LeafFieldSaver(conv, pathPrefix, clazz, field, ignoreClassIndexing, collectionize);
				this.fieldSavers.add(saver);
			}
		}
	}
