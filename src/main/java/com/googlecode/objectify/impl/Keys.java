...
public class Keys
{
	 ...
	 */
	public com.google.cloud.datastore.Key createRaw(final com.google.cloud.datastore.Key parent, final String kind, final long id) {
		if (parent == null) {
	 ...
	 * @param parent can be null for root keys
	 */
	public com.google.cloud.datastore.Key createRaw(final com.google.cloud.datastore.Key parent, final String kind, final String name) {
		if (parent == null) {
			return datastore.newKeyFactory().setKind(kind).newKey(name);
		} else {
	 ...
	 * @param parent can be null for root keys
	 */
	public IncompleteKey createRawIncomplete(final com.google.cloud.datastore.Key parent, final String kind) {
		if (parent == null) {
			return datastore.newKeyFactory().setKind(kind).newKey();
		} else {
	 ...
	 * @param id must be either Long or String
	 */
	public <T> Key<T> createKeyAny(final Key<?> parent, final Class<T> kind, final Object id) {
		if (id instanceof String)
			return createKey(parent, kind, (String)id);
		else if (id instanceof Long)
			return createKey(parent, kind, (Long)id);
		else
			throw new IllegalArgumentException("id '" + id + "' must be String or Long");
	}
	 ...
	 */
	public <T> List<Key<T>> createKeys(final Key<?> parent, final Class<T> kind, final Iterable<?> ids) {
		return StreamSupport.stream(ids.spliterator(), false)
				.map(id -> createKeyAny(parent, kind, id))
